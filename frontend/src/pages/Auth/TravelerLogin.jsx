  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (loginData.user.likedRooms) {
        ...
        localStorage.setItem('likedHomes', JSON.stringify([]));
      }
      
      if (loginData.user.viewedRooms) {
        // Store history as array of room IDs for backward compatibility
        const historyRoomIds = loginData.user.viewedRooms.map(entry => entry.roomId || entry);
        localStorage.setItem('viewedHomes', JSON.stringify(historyRoomIds));
      } else {
